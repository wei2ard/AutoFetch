  ...
  for param in ${!params[@]}; do
    ...
    if [[ ${user} == WE1ZARD ]]; then # release
        ...
        if [[ ${pre} > 0 ]]; then # pre-release 
            curl -sL -H "Authorization: token $TOKEN" -sL "$API/${user}/${repo}/releases" | jq '.[]' | jq 'select(.prerelease)' | jq -r '.assets.[].url' | xargs -I {} curl -sL -H "Accept: application/octet-stream" -H "Authorization: Bearer $TOKEN" {} -o ${file}"-pre"${type}; 
